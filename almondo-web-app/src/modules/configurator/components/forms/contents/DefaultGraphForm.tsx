...
const DefaultGraphForm = () => {
  ...
  const { parameters, handleParameterChange, handleSubmit } = useParametersState(graphType);
    ...
    <form className="w-2/3 mt-4" onSubmit={(e) => handleSubmit(e)}>
      ...
      {parameters.getParams().map((parameter) => (
        <div key={parameter.label} className="mb-5">
          ...
          <input
            ...
            type="number"
            step={parameter.label === "Probability" || parameter.label === "Rewiring Probability" ? 0.1 : 1}
      ...
      <div className="flex justify-center">
        <button
          ...
          className="focus:outline-none text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 mt-4"
        >
          Generate
        </button>
      </div>
